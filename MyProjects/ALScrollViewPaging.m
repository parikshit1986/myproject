...
- (void)setHasPageControl:(BOOL)hasPageControl {
    ...
    if (hasPageControl) {
        ...
        CGFloat pageControlX = scrollViewCenterPointX - (pWidth / 2);
        CGFloat pageControlY = self.frame.origin.y + self.frame.size.height + 5;
...
- (void)scrollViewDidScroll:(UIScrollView *)sender {
	if (!pageControlBeingUsed) {
		...
		CGFloat page = floor((self.contentOffset.x - pageWidth / 2) / pageWidth) + 1;
